...
public class Board {
    ...
    public void printBoard() { //Wyświetla planszę w konsoli
        for (int i = 0; i < tiles.length; i++) {
            if (i != 0 && i % Settings.getBoardSize() == 0)
                System.out.println();
    ...
    public void generateNumbers() {
        ...
        for (int j = 0; j < Settings.getBoardSize(); j++) {
            ...
            for (int i = 0; i < Settings.getBoardSize(); i++) {
                ...
                if (tiles[j * Settings.getBoardSize() + i].isChecked()) //zlicza zamalowane kafelki z rzędu
                {
                    inrow++;
                } else if (i != 0 && tiles[j * Settings.getBoardSize() + i - 1].isChecked()) {  //warunek do dodawania kolejnych dlugości kafelków z rzędu, jezeli nie jest pierwsza i kafelek przed nia jest checked (jest co zapisac)
                    array.add(inrow);
                    inrow = 0;
                }
                if (i == Settings.getBoardSize() - 1 && tiles[j * Settings.getBoardSize() + i].isChecked()) {
        ...
        for (int j = 0; j < Settings.getBoardSize(); j++) {
            ...
            for (int i = 0; i < Settings.getBoardSize(); i++) {
                ...
                if (tiles[j + i * Settings.getBoardSize()].isChecked()) //zlicza zamalowane kafelki z rzędu w kolumnie
                {
                    incol++;
                } else if (i != 0 && tiles[j + (Settings.getBoardSize()) * (i-1)].isChecked()) {  //warunek do dodawania kolejnych dlugości kafelków z kolumny, jezeli nie jest pierwsza i kafelek nad nim jest checked (jest co zapisac)
                    array1.add(incol);
                    incol = 0;
                }
                if (i == Settings.getBoardSize()-1 && tiles[j + Settings.getBoardSize() * i].isChecked()) {
